...
interface EmployeeCommentCardProps {
  commentId: string;
  name: string;
  time: Date;
  comment: string;
...
export default function EmployeeCommentCard({
  commentId,
  name,
  time,
  comment,
  ...
  const { mutate: updateEmployeeComment } =
    api.employeeComments.update.useMutation({
      onSuccess: () => {
  ...
  const { mutate: removeEmployeeComment } =
    api.employeeComments.delete.useMutation({
      onSuccess: () => {
        // Refresh the page to show updated comments (since comments are server-rendered)
        router.refresh();
        toast({
  ...
  const handleRemoveComment = () => {
    removeEmployeeComment({ commentId: commentId });
  };

  const handleUpdateComment = () => {
    updateEmployeeComment({ commentId: commentId, text: updatedComment });
    setIsEditing(false);
  };
