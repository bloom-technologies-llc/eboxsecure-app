...
interface LocationMentionDropdownProps {
  showMentions: boolean;
  setShowMentions: (show: boolean) => void;
  mentionPosition: { x: number; y: number } | null;
  locationEmployees:
    | RouterOutputs["locations"]["getLocationEmployees"]
    | undefined;
...
export default function LocationMentionDropdown({
  showMentions,
  setShowMentions,
  mentionPosition,
  locationEmployees,
          ...
          {locationEmployees
            ...
            .map((employee, index) => {
              return (
                ...
                >
                  <p className="text-sm">Employee {employee.id}</p>
                </div>
              );
            })}
        </div>
