...
import CustomerMentionDropdown from "./customer-mention-dropdown";

interface CustomerCommentFormContainerProps {
  customerId: string;
}
...
export default function CustomerCommentFormContainer({
  customerId,
}: CustomerCommentFormContainerProps) {
  ...
  // For now, use empty array for admin users - mentions will be implemented later
  // when proper admin user endpoint is created
  const adminUsers: any[] = [];

  const createCommentMutation =
    api.customers.customerComments.create.useMutation({
      onSuccess: () => {
        router.refresh();
        utils.customers.customerComments.query.invalidate({ customerId });
        toast({
  ...
  const onSubmit = async (values: z.infer<typeof commentSchema>) => {
    ...
    createCommentMutation.mutate({
      ...
      notifications: mentionedUsers.map((mentionedUser) => ({
        ...
        message: `${mentionedUser.display} was mentioned in a comment`,
      })),
    });
                ...
                <FormControl>
                  <div className="flex flex-col gap-y-2">
                    ...
                    <Textarea
                      ...
                      className="rounded-none border border-none px-4 py-6 focus:ring-0"
                      onKeyDown={(e) => handleKeyDown(e, adminUsers)}
                      onChange={(e) => {
                        field.onChange(e);
                        handleInputChange(
                          e.target.value,
